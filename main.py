...
BOT_TOKEN = "5416695195:AAEzZccf2wp4ZSqa3hnIMazcr7quyjRwyXk"
PAYMENT_CHANNAL = "@airdrop_payments" #add payment channel here including the '@' sign
OWNER_ID = 1237699122 #write owner's user id here.. get it from @MissRose_Bot by /id
CHANNELS = ["@Meta_Axis","@NB_Airdrops","@airdrop_payments"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Set Wallet":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Send your MTA Wallet Address._",
                                parse_mode="Markdown", reply_markup=keyboard)
        # Next message will call the name_handler function
        bot.register_next_step_handler(message, MTA_address)
...
def MTA_address(message):
   try:
    ...
    if len(message.text) == 34:
        ...
        bot.send_message(message.chat.id, "*💹Your MTA wallet set to " +
                         data['wallet'][user]+"*", parse_mode="Markdown")
        json.dump(data, open('users.json', 'w'))
        return menu(message.chat.id)
    else:
        bot.send_message(
            message.chat.id, "*⚠️ It's Not a Valid MTA Address!*", parse_mode="Markdown")
        return menu(message.chat.id)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
